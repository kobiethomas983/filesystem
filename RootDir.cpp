...
bool RootDir::AddFile (string name, int size, blockNum location) {
	if (FindFile(name) == -1) {
		cout << "I am in the addfile funtion" << endl;
		...
		cout << this->rootHeader.fileBlockLocs[this->count] << endl;
		this->count++;
		return true;
	}
	else
...
blockNum RootDir::FindFile(string searchKey) const {
	if (this->count == 0){
		cout << "IM in the this->count" << endl;
		return -1;
	}	
	else {
		cout << "im in the else state findFile" << endl;
		FileNode* pCurr = this->pHead;
